...
def norm_sum_df(df, reduction_factor, stocks):
    new_df = pd.DataFrame(index=df.index)
    for stock in stocks:
        rolling_sum = df[stock].rolling(window=int(1 / (1 - reduction_factor)), min_periods=1).apply(lambda x: np.dot(x, reduction_factor ** np.arange(len(x) - 1, -1, -1)))
        new_df[stock] = rolling_sum
    return new_df
#integral.plot()
...
def decision_1(integral, df_price, stocks, up_thresh=0.9, low_thresh=0.05, thresh_window=180, clipping = 10):
    ...
    for stock in stocks:
        ...
        new_df['up_bound'] = integral[stock].rolling(thresh_window,min_periods = 0).quantile(up_thresh)
        new_df['low_bound'] = integral[stock].rolling(thresh_window,min_periods = 0).quantile(low_thresh)

        cooler_counter = np.arange(len(integral[stock]))
        trade = np.zeros(len(integral[stock]))
        #buy_trig, sell_trig = np.zeros(2)

        sell_mask = np.array(integral[stock]) > np.array(new_df['up_bound'])
        buy_mask = np.array(integral[stock]) < np.array(new_df['low_bound'])
        
        sell_trig = np.where(sell_mask, False, 1)
        buy_trig = np.where(buy_mask, False, -1)
        
        
        #new_df['trade'] = trade
        all_trades = (-buy_trig-1) + (1 - sell_trig)
        all_trades[:clipping] = 0
        decisions[stock] = all_trades
   
    return decisions
# decide_1 = decision_1(integral = integral, df_price = df_price, stocks = stocks,up_thresh = 0.6, low_thresh = 0.1, thresh_window = 180, clipping = 10)

def trade_price_df(price_df, decision_trade_df, brokerage_fee=20, trade_amount=6000, compound_factor=None, bank=8000, mode='percentage', stocks=None):
    bank_initial = bank
    new_df = pd.DataFrame(index=decision_trade_df.index, columns=stocks, dtype=np.float64)
    ...
    for stock in stocks:
        bank = bank_initial
        trade_amount = bank_initial
        holding_stocks = 0
        bank_value = np.zeros_like(price_df[stock].values, dtype=np.float64)
        total_value = np.zeros_like(price_df[stock].values, dtype=np.float64)
        
        for i, decision in enumerate(decision_trade_df[stock]):
            price = price_df[stock][i]
            if decision == -1 and bank >= trade_amount:
                bank -= trade_amount
                holding_stocks += (trade_amount - brokerage_fee) / price
                if compound_factor is not None:
                    trade_amount = compound_factor * bank

            elif decision == 1 and holding_stocks > 0:
                bank += holding_stocks * price - brokerage_fee
                holding_stocks = 0
                if compound_factor is not None:
                    trade_amount = compound_factor * bank

            bank_value[i] = bank
            total_value[i] = bank + holding_stocks * price

        bank_value[-1] = bank + holding_stocks * price_df[stock].values[-1]
        ...
        if mode == 'value':
            ...
            new_df[stock] = ((total_value / bank_initial) - 1) * 100

    return new_df

### Ch 3.2

def norm_int_df(norm_df = df , stocks = stocks):
...
def decision_2(norm_int_df =df , norm_sum_df = df , limit_factor = 1, stocks = stocks,
                     trig_thresh = 20):
...
### Ch 3.3
def integral_bound(integral = df , stocks = stocks, 
...
def decision_3(integral_bound = df , stocks = stocks, trig_thresh = 20):
...
def moving_mean(df_price = df , stocks = stocks, mode = 'mean', window = 50):
...
def moving_bounds(df_price = df, moving_df = df, stocks = stocks, deviations = 2, mode = 'mean', window = 50):
    new_df = pd.DataFrame(index = df_price.index)
    for stock in stocks:
...
def decision_4(df_price = df , moving_b = df , stocks = stocks, trig_thresh = 20):
    new_df = pd.DataFrame(index = df_price.index)
    for stock in stocks:
        decide_array = []
        buy_trig, sell_trig = 0, 0
...
def sharpe_list(df = df , ascend = True, stock_num = None , days = 252, window_days = 90):
    df_bound = df[:days]
...
def sortino_list(df = df  , ascend = True, stock_num = None , days = 252, window_days = 90):
    df_bound = df[:days]
    log_returns = np.log(df_bound/df_bound.shift(1)).dropna()
    daily_std = log_returns.std()
    annualised_vol = daily_std * np.sqrt(252) * 100
...
def trade_price_all_df(price_df  = df, decision_trade_df = df , brokerage_fee = 20, 
...
def final_results(df, stocks, output=False):
    final_array = [round(df[stock][-1], 5) for stock in stocks]
    ...
    if output:
        ...
        print(f'Average: {mean:.2f} ± {std:.2f}, C_Var: {cvar:.2f}')
    return [round(cvar, 2), round(mean, 2), round(std, 2)]

def compare_results(df_1 = df, df_2 = df, stocks = stocks, mode = 'percentage'):
    df_1_results = []
    df_2_results = []
    for stock in stocks:
...
def risk_stats(decide = df , df_price = df , stocks = stocks, mode = 'value', 
...
def plotting_results(df_price = df, integral = df, decide = df , price = df , 
...
def compare_graphs(df1 = df , df2 = df , stocks = stocks):

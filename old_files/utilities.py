...
# Use the Daily Opening data# Define Stocks of interest# Define Start and End dates
...
def final_results(df, stocks = ['CBA.AX'], output = False):
    final_array = []
    for stock in stocks:
...
def risk_stats(decide = None, df_price = df_price, stocks = ['CBA.AX'], mode = 'value', 
...
def plotting_results(df_price = df_price, integral = integral, decide = decide_1, price = price_1, 
                     stocks = ['CBA.AX']):
    over_all = risk_stats(decide = decide, df_price = df_price, compound_factor = 1, output = False)
...
def compare_graphs(df1 = price_1, df2 = price_2, stocks = ['CBA.AX']):
...
def buy_n_hold(p_df, stocks = ['CBA.AX'], mode = 'value', total_investment = 8000, brokerage_fee = 20):
    price_df = p_df
...
def doll_cost_av(p_df = df_price, day_freq = 60, trade_amount = 5000, stocks = ['CBA.AX'], brokerage_fee = 20, 
                 mode = 'value'):
...
def norm_sum_df(df = gradient, reduction_factor  = 1 , stocks = ['CBA.AX']):
    ...
    for stock in stocks:
        ...
        new_df[stock] = totals
    return new_df
integral = norm_sum_df(df = gradient, reduction_factor = 0.9, stocks = ['CBA.AX'])
#integral.plot()
...
def decision_1(integral = integral, df_price = df_price, stocks = ['CBA.AX'], 
    ...
    return decisions
#decide = decision_1(integral = integral, df_price = df_price, stocks = stocks_var,up_thresh = 0.6, low_thresh = 0.4, thresh_window = 180,
#                    trig_thresh = 1)
#decide.plot()
...
def decision_2(norm_int_df = intercept, norm_sum_df = integral, limit_factor = 1, stocks = ['CBA.AX'],
                     trig_thresh = 20):
...
### Ch 3.3
def integral_bound(integral = integral, stocks = ['CBA.AX'], 
...
def decision_4(integral_bound = integral_b, stocks = ['CBA.AX'], trig_thresh = 20):
...
def moving_mean(df_price = df, stocks = ['CBA.AX'], mode = 'mean', window = 50):
...
def decision_4(df_price = df, moving_b = moving_b, stocks = ['CBA.AX'], trig_thresh = 20):
    new_df = pd.DataFrame(index = df_price.index)
    for stock in stocks:
        decide_array = []
        buy_trig, sell_trig = 0, 0
...
def sharpe_list(df, ascend = True, stock_num = None, days = 252, window_days = 90):
    df_bound = df[:days]
    ...
    Rf = 0.01/252
    sharpe_ratio = (log_returns.rolling(window = window_days).mean() - Rf*trading_days/volatility)
    return list(sharpe_ratio.median().sort_values(ascending = ascend)[:stock_num].index)
#sharpe_list(df, ascend = True, stock_num = 4)
def sortino_list(df, ascend = True, stock_num = None, days = 252, window_days = 90):
    df_bound = df[:days]
    log_returns = np.log(df_bound/df_bound.shift(1)).dropna()
    daily_std = log_returns.std()
    annualised_vol = daily_std * np.sqrt(252) * 100
    ...
    sortino_ratio = (log_returns.rolling(window = trading_days).mean() - Rf*window_days/sortino_vol)
    return list(sortino_ratio.median().sort_values(ascending = ascend)[:stock_num].index)
#sortino_list(df, ascend = True, stock_num = 4)

##### Ch 5: Ensemble Algorithms
def trade_price_all_df(price_df = df_price, decision_trade_df = df, brokerage_fee = 20, 
                   trade_amount = 6000, compound_factor = None, bank = 8000, mode = 'percentage',
                  stocks = ['CBA.AX'], decide_thresh = 1):
    bank_initial = bank
    print('Trade Output:', mode)
    new_df = pd.DataFrame(index = decision_trade_df.index)
